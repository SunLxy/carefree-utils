...
const calendar = {
  ...
  getFestival(
    ...
    const obj = FestivalArray.filter((item: any) => {
      ...
      const flag4 =
        ...
        lunarMonth === item.lunarMonth &&
        lunarDay === item.lunarDay &&
        lunarDay === calendar.monthDays(lunarYear, lunarMonth) &&
        !!item.newYearEve;
  ...
  solar2lunar(
    ...
    // 依据12节气修正干支月
    let gzM = calendar.totianGandiZhi(
      (solarYear - 1900) * 12 + solarMonth + 11,
    );
    ...
    const lunarMonthDays = isLeap
      ? calendar.leapDays(lunarYear)
      : calendar.monthDays(lunarYear, lunarMonth);
    ...
    // 阴历月份的中文
    const IMonthCn =
      (isLeap ? '\u95f0' : '') + calendar.toChinaMonth(lunarMonth);
  ...
  lunar2solar(
    ...
    if (date > _day) {
      const error = `传参要求计算阴历月份的天数，该阴历月份并没有阴历${calendar.toChinaDay(
        date,
      )}`;
      return error;
    } //参数合法性效验
...
calendar.solar2lunar(2021, 11, 1);
